...
<div class="container-fluid">
    <div class="row">
            ...
            <div class="list-group div-scroll">
                ...
                <div>
                    <div class="form-group">
                        ...
                        <form method="get">
                            <select class="form-control" name="filter-label[]" id="filter-label" multiple>
                    ...
                    @php
                        $filterLabels = collect(request('filter-label',[]));
                        if ($filterLabels->count() > 0){
                        \Illuminate\Support\Facades\Cache::forever('filter-label',serialize($filterLabels));

                        }else
                            {
                                $filterLabels = unserialize(\Illuminate\Support\Facades\Cache::get('filter-label',serialize(collect())));
                            }
                        $themeByLabel = [
                            'none'=> 'secondary',
      'emergency'=> 'danger',
      ...
      'debug'=> 'danger',
                    ...
                    @endphp
                    <div>
                        @foreach($filterLabels as $item)
                            <span style="font-size: 13px!important;"
                                  class="badge badge-{{$themeByLabel[$item]}}">{{$item}}</span>
                        @endforeach
                    </div>
                </div>
            </div>
            ...
            @else
                <table id="table-log" class="table table-striped" data-ordering-index="{{ $standardFormat ? 2 : 0 }}">
                    ...
                    @foreach($logs as $key => $log)
                        @if($filterLabels->contains($log['level']))
                            @continue
                        @endif
